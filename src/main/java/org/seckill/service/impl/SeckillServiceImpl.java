...
public class SeckillServiceImpl implements SeckillService {
    ...
    //md5盐值，用于混淆md5
    private final String slat = "aweriuh129y984e12y4ebjk124nbekj12&^*(^&";
    ...
    public SeckillExecution executeSeckill(Long seckillId, String userPhone, String md5) throws SeckillException, SeckillCLoseException, RepeatKillException {
        ...
        try {
            Integer updateCount = seckillDao.redeceNumber(seckillId, nowTime);
            if(updateCount <= 0) {
                //没有更新到记录，秒杀结束
                throw new SeckillCLoseException("seckill is closed.");
            } else {
                //记录购买行为
                Integer insertCount = successKilledDao.insert(seckillId, userPhone);
                if(insertCount <= 0) {
                    //重复秒杀
